...
import services.sahar.ServiceParticipation;
...
public class AfficherEmplEventContoller {
    ...
    private void ouvrirAjoutParticipation(int idEvent) {
        ...
        if (accessToken == null || !JWTUtils.isTokenValid(accessToken)) {
            showAlert("Erreur", "Session expirée. Veuillez vous reconnecter.", Alert.AlertType.ERROR);
            return;
        }

        User currentUser = JWTUtils.validateAccessToken(accessToken);
        if (currentUser == null) {
            showAlert("Erreur", "Impossible de récupérer les informations de l'utilisateur.", Alert.AlertType.ERROR);
            return;
        }

        int idUser = currentUser.getId(); // Récupérer l'ID utilisateur connecté
        ...
        try {
            if (serviceParticipant.checkParticipation(idEvent, idUser)) {
                ...
                return; // Arrête l'exécution si l'utilisateur est déjà inscrit
            }

            FXMLLoader loader = new FXMLLoader(getClass().getResource("/employe/sahar/AjoutParticipation.fxml"));
