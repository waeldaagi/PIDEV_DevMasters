...
public class GoogleCalendar {
    private static final String APPLICATION_NAME = "TripToGo";
    private static final JsonFactory JSON_FACTORY = GsonFactory.getDefaultInstance();
    private static final String CREDENTIALS_FILE_PATH = "/client_secret.json";
    private static final String TOKENS_DIRECTORY_PATH = System.getProperty("user.home") + "/triptogo_tokens";
    ...
    private static Calendar getCalendarService() throws IOException, GeneralSecurityException {
        ...
        // Try different ports for the local server receiver
        LocalServerReceiver receiver = null;
        Credential credential = null;
        int[] ports = {8888, 8889, 8890, 8891, 8892}; // Try multiple ports
        ...
        for (int port : ports) {
            try {
                ...
                System.out.println("Attempting to authorize using port " + port);
                credential = new AuthorizationCodeInstalledApp(flow, receiver)
                        .authorize("user");
                System.out.println("Successfully obtained credentials using port " + port);
                ...
                // If this was the last port, throw the exception
                if (port == ports[ports.length - 1]) {
                    throw new IOException("Failed to authorize. All ports are in use. Please try again in a few minutes or close other applications that might be using these ports.");
                }
            }
        }

        if (credential == null) {
            throw new IOException("Failed to obtain credentials after trying all available ports.");
        }
    ...
    public static void AjouterProjetsDansCalendrier() throws IOException, GeneralSecurityException {
        ...
        try {
            ...
            if (projets == null || projets.isEmpty()) {
                throw new IOException("No projects found in the database.");
            }

            System.out.println("Found " + projets.size() + " projects to process.");

            // Process each project
            for (Projet projet : projets) {
                Event event = createEventFromProjet(projet);
                try {
                    event = service.events().insert(CALENDAR_ID, event).execute();
                    System.out.printf("Successfully created event: %s (%s)\n", event.getSummary(), event.getHtmlLink());
                } catch (IOException e) {
                    System.err.printf("Failed to create event for project %d: %s\n",
                            projet.getId_projet(), e.getMessage());
                }
            }
        } catch (Exception e) {
            System.err.println("Error in AjouterProjetsDansCalendrier: " + e.getMessage());
            e.printStackTrace();
            throw new IOException("Failed to add projects to calendar: " + e.getMessage(), e);
        }
    }

    private static Event createEventFromProjet(Projet projet) {
        Event event = new Event();
        event.setSummary(projet.getNom_projet());
        ...
        java.sql.Date sqlDate = projet.getDeadline();
        LocalDate localDate = sqlDate.toLocalDate();
        LocalDateTime localDateTime = localDate.atStartOfDay();
        Instant instant = localDateTime.atZone(ZoneId.systemDefault()).toInstant();

        DateTime startDateTime = new DateTime(instant.toEpochMilli());
        DateTime endDateTime = new DateTime(instant.toEpochMilli() + 24 * 60 * 60 * 1000); // Add 24 hours
    ...
    public static void AfficherCalendrierComplet() throws IOException, GeneralSecurityException {
        Calendar service = getCalendarService();
        ...
        try {
            ...
            Events events = service.events().list(CALENDAR_ID)
                    ...
                    .setShowDeleted(false)  // Don't show deleted events
                    .execute();
            ...
            if (items != null) {
                items.sort((e1, e2) -> {
                    long start1 = e1.getStart().getDateTime().getValue();
                    long start2 = e2.getStart().getDateTime().getValue();
            ...
            Label titleLabel = new Label("Project Calendar");
            ...
            Label upcomingTitle = new Label("Upcoming Project Deadlines");
            ...
            if (items.isEmpty()) {
                Label noEvents = new Label("No upcoming project deadlines");
                ...
                for (Event event : items) {
                    ...
                    Label timeText = new Label("📅 Deadline: " + start.toString().substring(0, 10));
                    timeText.getStyleClass().add("upcoming-event-time");

                    Label descriptionText = new Label(event.getDescription());
                    descriptionText.getStyleClass().add("upcoming-event-location");
                    descriptionText.setWrapText(true);

                    eventBox.getChildren().addAll(titleText, timeText, descriptionText);
            ...
            calendarStage.setTitle("Project Calendar");
